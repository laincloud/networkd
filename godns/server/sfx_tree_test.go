...
func Test_Suffix_Tree(t *testing.T) {
	...
	Convey("Google should not be found", t, func() {
		...
		So(setToStringSlice(v), ShouldResemble, []string{"166.111.8.28"})
	})
	...
	Convey("Google should be found", t, func() {
		...
		v, found := root.search(strings.Split("google.com", "."))
		So(found, ShouldEqual, true)
		So(setToStringSlice(v), ShouldResemble, []string{"8.8.8.8"})
		...
		So(found, ShouldEqual, true)
		So(setToStringSlice(v), ShouldResemble, []string{"208.67.222.222"})
